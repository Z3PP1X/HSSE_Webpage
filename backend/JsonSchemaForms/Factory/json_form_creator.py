...
class MultiModelFormCreator(JsonFormCreator, ModelMetadataProvider):
    ...
    def _get_model_metadata_for_instance(self, model_class: models.Model,
        ...
        for field_name, form_field in form_fields.items():
            try:
                ...
                default_label = (f"{instance_label} - "
                                 f"{model_field.verbose_name or field_name.replace('_', ' ').title()}")
    ...
    def configure_from_dict(self, config: dict):
        """Configure the form schema from a dictionary with
        support for custom questions and expandable categories."""
        # Apply AJAX configs
        if 'ajax_configs' in config:
            for key, cfg in config['ajax_configs'].items():
                self.add_ajax_config(
                ...
                for field_spec in cat.get('fields', []):
                    if isinstance(field_spec, str):
                        if field_spec in self.all_model_fields:
                            field_info = (
                                self.all_model_fields[field_spec].copy())
                            field_configs.append(field_info)
                    elif isinstance(field_spec, dict):
                        field_info = self._process_field_spec(
                            field_spec,
                            is_expandable, cat['key'])
                        if field_info:
                            field_configs.append(field_info)
    ...
    def _process_field_spec(self, field_spec: dict,
                            is_expandable: bool = False,
                            category_key: str = None) -> Union[Dict, None]:
        ...
        if 'instance' in field_spec and 'field' in field_spec:
            ...
            if prefixed_name in self.all_model_fields:
                ...
                if 'ajax' in field_spec:
                    ajax_key_base = (
                        field_spec['question']
                        if 'question' in field_spec else prefixed_name)
                    if is_expandable:
                        ajax_key_base = f"{ajax_key_base}_{{index}}"
                    self._apply_ajax_config(
                        field_info, field_spec['ajax'], ajax_key_base)
        ...
        # Legacy support for model-based specification
            ...
            for instance_key, instance_config in self.model_instances.items():
                if instance_config['model'].__name__ == model_name:
                    ...
                    if prefixed_name in self.all_model_fields:
                        field_info = (
                            self.all_model_fields[prefixed_name].copy())
    ...
    def _apply_ajax_config(self, field_info: dict,
                           ajax_config: dict, field_key: str):
